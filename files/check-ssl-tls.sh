...
# script: ssltls.check 
# version: 1.1
# author: Simon Kowallik <sk simonkowallik.com>, Andre Schild <a.schild aarboard.ch>
# description: 
# license: GPL2
#
# DETAIL:
...
# there need to be exactly 3 arguments, if not, exit 1 (error for console)
if [ $# -ne 3 ]; then
 exit 1
fi
...
TLS_TYPE=$2 # native or proto (which means we use starttls)
CHECK_TYPE=$3 # simple, lifetime, .., serial, .., issuer, ..
...
# 2018-02-21, mm: 10 -> 20
TIMEOUT=20 # connect timeout in seconds
...
case $CHECK_TYPE in
  "simple")
    ...
    # execute command
    RETURN=$($echo_bin | $timeout_bin $TIMEOUT $OPENSSL s_client -connect $HOST $STARTTLS 2>/dev/null | $OPENSSL x509 -noout $X509_OPT 2>/dev/null)
  ...
  "digestmode")
    X509_OPT="-text"
    RETURN=$($echo_bin | $timeout_bin $TIMEOUT $OPENSSL s_client -connect $HOST $STARTTLS 2>/dev/null | $OPENSSL x509 -noout  $X509_OPT  2>/dev/null | grep "Signature Algorithm")
  ...
  "ssl3")
    X509_OPT="-text"
    RETURN=$($echo_bin | $timeout_bin $TIMEOUT $OPENSSL s_client -connect $HOST $STARTTLS -ssl3 2>&1 >/dev/null | grep "handshake failure")
    searchString="sslv3 alert handshake failure"
    searchString=2"ssl handshake failure"
    if [ "$RETURN" == "" ]; then
